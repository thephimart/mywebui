...
def upgrade() -> None:
    op.create_table(
        'users',
        sa.Column('id', sa.dialects.sqlite.UUID(as_uuid=True), nullable=False),
    ...
    op.create_table(
        'documents',
        sa.Column('id', sa.dialects.sqlite.UUID(as_uuid=True), nullable=False),
        sa.Column('owner_id', sa.dialects.sqlite.UUID(as_uuid=True), nullable=False),
        sa.Column('title', sa.String(500), nullable=False),
        sa.Column('visibility', sa.Enum('public', 'restricted', 'private', name='visibility_type'), nullable=False, server_default='private'),
    ...
    op.create_table(
        'chunks',
        sa.Column('id', sa.dialects.sqlite.UUID(as_uuid=True), nullable=False),
        sa.Column('document_id', sa.dialects.sqlite.UUID(as_uuid=True), nullable=False),
        sa.Column('modality', sa.Enum('text', 'image', 'audio', 'video', name='modality_type'), nullable=False),
    ...
    op.create_table(
        'embeddings',
        sa.Column('id', sa.dialects.sqlite.UUID(as_uuid=True), nullable=False),
        sa.Column('chunk_id', sa.dialects.sqlite.UUID(as_uuid=True), nullable=False),
        sa.Column('model_name', sa.String(100), nullable=False),
        sa.Column('modality', sa.Enum('text', 'image', name='embedding_modality'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('user_id', sa.dialects.sqlite.UUID(as_uuid=True), nullable=True),
...
def downgrade() -> None:
    ...
    op.execute('DROP TYPE IF EXISTS visibility_type')
    op.execute('DROP TYPE IF EXISTS modality_type')
    op.execute('DROP TYPE IF EXISTS embedding_modality')
